...
void BmpSave(char *file,unsigned char *buff,unsigned int x,unsigned int y,unsigned int b)
{
	...
	int m,n,q,p;
	printf("请依次输入需要裁剪的宽，需要裁剪的高，裁剪的初始高度，裁剪的初始宽度\n");
	scanf("%d%d%d%d",&n,&m,&q,&p);
    int ix=x-n;
	int iy=y-m;
	bbuff=(unsigned char *)malloc(ix*iy*b);
	// 文件现在的设定
	tbuff[0] = 'B';
	tbuff[1] = 'M';
	fwrite(tbuff,2,1,fp);
	tbuff[3] = (14 +40 +ix*iy*b >> 24) & 0xff;//右移24位，取低8位
	tbuff[2] = (14 +40 +ix*iy*b >> 16) & 0xff;//右移16位，取低8位
	tbuff[1] = (14 +40 +ix*iy*b >>  8) & 0xff;//右移8位，取低8位
	tbuff[0] = (14 +40 +ix*iy*b >>  0) & 0xff;//右移0位，取低8位
	fwrite(tbuff,4,1,fp);
	tbuff[1] = 0;
	tbuff[0] = 0;
	fwrite(tbuff,2,1,fp);
	...
	// 信息的设定
	lpBi.biSize				= 40;    //本结构所占用字节数
	lpBi.biWidth			= ix;     //位图的宽度
	lpBi.biHeight			= iy;     //位图的高度
	lpBi.biPlanes			= 1;     //目标设备的级别，必须为1
	lpBi.biBitCount			= b*8;   //每个像素所需的位数
	lpBi.biCompression	 	= 0;     //位图压缩类型，必须是0
	lpBi.biSizeImage		= ix*iy*b; //位图的大小
	lpBi.biXPelsPerMeter	= 300;   //位图水平分辨率，每米像素数
	lpBi.biYPelsPerMeter	= 300;   //位图垂直分辨率，每米像素数
	lpBi.biClrUsed		 	= 0;     //位图实际使用的颜色表中的颜色数
	lpBi.biClrImportant		= 0;     //位图显示过程中重要的颜色数
	fwrite(&lpBi,1,40,fp);
    ...
    for(k=0;k<iy;k++)
	{
		for(i=0;i<ix*3;i++)
		{
		    bbuff[k*ix*3+i]=buff[(k+q)*x*3+i+p*3];
		}
	}

    fwrite(bbuff,1,ix*iy*b,fp);
	free(bbuff);
	fclose(fp);
}
...
/** 32位数据取得(仅在解压缩时使用)
* JPEG中以0xFF来作为特殊标记字符，
* 如果某个像素的值为0xFF那么实际在存放的时候是以0xFF00来保存的，
* 从而避免了其跟特殊字符0xFF之间产生混淆
**/
...
const int C1_16 = 4017; 			
const int C2_16 = 3784; 		
const int C3_16 = 3406; 				
const int C4_16 = 2896; 				
const int C5_16 = 2276; 				
const int C6_16 = 1567; 				
const int C7_16 = 799;	
