...
#if 0
	str = get_byte(buff);		// (5文字,"JFIF"[00])
	str = get_byte(buff);
	str = get_byte(buff);
	str = get_byte(buff);
	str = get_byte(buff);
	data = get_word(buff);		// 稳定版本
	str = get_byte(buff);		// 分辨率单位
	data = get_word(buff);		// 横方向的分辨率
	data = get_word(buff);		// 纵方向的分辨率
	data = get_word(buff);		// 缩略图的横向像素数
	data = get_word(buff);		// 缩略图的竖像素数
	data = get_word(buff);		// 缩略图(仅在某些情况下)
#endif
}
...
void HuffmanDecode(unsigned char *buff, unsigned char table, int *BlockData)
{
	...
	while(count < 64)
	{
		...
		//printf(" Use Bit: %d\n",(i + DataCount +1));			// 输出使用的位数（码字位数加数据的位数）
...
int main(int argc, char* argv[])
{
	...
	if((fp = fopen(argv[1], "rb")) == NULL)		//打开待解码的JPG图片
	{
		// perror(s)用来将上一个函数发生错误的原因输出到标准错误(stderr)
	...
	BmpSave(argv[2],Buff,BuffX,BuffY,3);			// Bitmap
